...
import * as mdsToken from '../src/token';
...
describe('Connection', function () {
  ...
  it('get program accounts', async () => {
...

    {
      await helpers.airdrop({
        connection,
        address: account1.publicKey,
        amount: 0.5 * LAMPORTS_PER_MUNDIS,
      });
        ...
        value: [
          {
            account: {
              data: ['', 'base64'],
              executable: false,
              lamports: LAMPORTS_PER_MUNDIS - feeCalculator.lamportsPerSignature,
          ...
          },
          {
            account: {
              ...
              executable: false,
              lamports:
                0.5 * LAMPORTS_PER_MUNDIS - feeCalculator.lamportsPerSignature,
              owner: programId.publicKey.toBase58(),
              rentEpoch: 20,
            },
      ...
      programAccounts.forEach(function (keyedAccount) {
        if (keyedAccount.pubkey.equals(account0.publicKey)) {
          expect(keyedAccount.account.lamports).to.eq(
            LAMPORTS_PER_MUNDIS - feeCalculator.lamportsPerSignature,
          );
        } else {
  ...
  if (process.env.TEST_LIVE) {
    describe('token methods', () => {
      ...
      before(async function () {
        ...
        const mintPubkey = await mdsToken.createMint(
          connection as any,
          payerKeypair,
        ...
        const tokenAccountPubkey = await mdsToken.createAccount(
          connection as any,
          payerKeypair,
          mintPubkey,
        ...
        const mintPubkey2 = await mdsToken.createMint(
          connection as any,
          payerKeypair,
          mintOwnerKeypair.publicKey,
          null, // freeze authority
        ...
        const tokenAccountPubkey2 = await mdsToken.createAccount(
          connection as any,
          payerKeypair,
          mintPubkey2,
          accountOwnerKeypair.publicKey,
        ...
        const tokenAccountDestPubkey = await mdsToken.createAccount(
          connection as any,
          payerKeypair,
          mintPubkey,
          accountOwnerKeypair.publicKey,
        ...
        testSignature = await mdsToken.transfer(
          connection as any,
          payerKeypair,
          tokenAccountPubkey,
  ...
  if (process.env.TEST_LIVE) {
    it('stake activation should return activating for new accounts', async () => {
      ...
      const MIN_STAKE_DELEGATION = LAMPORTS_PER_MUNDIS;
  ...
  it('getVersion', async () => {
    ...
    expect(version['mundis-core']).to.be.ok;
  });
  ...
  it('request airdrop', async () => {
    ...
    expect(accountInfo.lamports).to.eq(LAMPORTS_PER_MUNDIS);
    ...
    expect(parsedAccountInfo.lamports).to.eq(LAMPORTS_PER_MUNDIS);
  ...
  it('transaction failure', async () => {
    ...
    let transaction = new Transaction().add(
      SystemProgram.createAccount({
        ...
        lamports: LAMPORTS_PER_MUNDIS / 2,
        space: 0,
  ...
  if (process.env.TEST_LIVE) {
    ...
    describe('given an open websocket connection', () => {
      ...
      it('account change notification', async () => {
        ...
        try {
          ...
          connection.requestAirdrop(owner.publicKey, LAMPORTS_PER_MUNDIS);
      ...
      it('logs notification', async () => {
        ...
        try {
          ...
          await connection.requestAirdrop(owner.publicKey, LAMPORTS_PER_MUNDIS);
    ...
    it('https request', async () => {
      const connection = new Connection('https://api.mainnet.mundis.io');
      const version = await connection.getVersion();
      expect(version['mundis-core']).to.be.ok;
    }).timeout(20 * 1000);
  }
